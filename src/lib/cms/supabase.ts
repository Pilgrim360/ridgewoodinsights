/**
 * Supabase CMS Client & Query Abstraction Layer
...
function formatSupabaseError(error: any): CmsError {
 ...
 * Converts Supabase errors to CmsError format
 */
// eslint-disable-next-line @typescript-eslint/no-explicit-any
export async function withErrorHandling<T>(
  ...
  try {
    ...
    if (error) {
      const cmsError = formatSupabaseError(error);
      throw new Error(cmsError.message);
    }
...
/**
 * Get user profile with cms status
 */
 ...
 */
export async function isCMS(): Promise<boolean> {
  try {
