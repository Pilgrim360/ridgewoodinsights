...
/**
 * Comprehensive error handler for cms CMS operations
 * Maps Supabase and app errors to user-friendly messages
 */

export class CmsErrorHandler {
  static parse(error: unknown): CmsError {
   ...
   */
  static getMessage(error: CmsError): string {
   ...
   */
  static isRetryable(error: CmsError): boolean {
   ...
   */
  static log(error: CmsError): void {
    console.error(`[CmsError: ${error.type}]`, error.message, error.originalError);
  }
}
