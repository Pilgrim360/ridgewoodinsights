...
export const AdvancedTableCell = TableCell.extend({
  addAttributes() {
    return {
      ...
      backgroundColor: {
        default: DEFAULT_CELL_STYLE_ATTRIBUTES.backgroundColor,
        parseHTML: (element) => normalizeHexColor((element as HTMLElement).style.backgroundColor),
      },
      textColor: {
        default: DEFAULT_CELL_STYLE_ATTRIBUTES.textColor,
        parseHTML: (element) => normalizeHexColor((element as HTMLElement).style.color),
      },
    };
  },
  ...
  renderHTML({ node, HTMLAttributes }) {
    ...
    return [
      ...
      mergeAttributes(HTMLAttributes, {
        class: ['rw-table-cell', HTMLAttributes.class].filter(Boolean).join(' '),
        style,
      }),
      0,
    ];
